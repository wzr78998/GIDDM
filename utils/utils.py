...
def flip_augmentation(data): # arrays tuple 0:(7, 7, 103) 1=(7, 7)
    ...
    #     data = np.flipud(data)
    #     data = torch.from_numpy(data.copy())
    # 轻微旋转的角度范围，例如 -10 到 10 度
    angle = np.random.uniform(-10, 10)  # 随机生成一个旋转角度
    ...
    for i in range(data.shape[0]):
        ...
        for j in range(data.shape[1]):
            ...
            # 轻微旋转当前通道的数据
            channel_data_rotated = np.rot90(channel_data, k=0)  # k=0 表示不改变数据的维度大小
            # 将旋转后的数据放入结果数组中
            rotated_data[i, j] = channel_data_rotated

    # 将 numpy 数组转换为 torch 张量
    augmented_data = torch.from_numpy(rotated_data)
...
def shuffle_aligned_list(data):
    ...
    num = data[0].shape[0]
    p = np.random.permutation(num)  # 随机排列序列。
...
def get_model_init(args, known_num_class, all_num_class):
    ...
    if net == 'vgg':
        ...
        model_g = DCRN_02(args.fea_dim,args.patches,model_name='DCRN_02')
    else:
...
def get_model(args, known_num_class, all_num_class, domain_dim=3, dc_out_dim=None):
    ...
    elif 'Cc2Net87' in net:
        model_g = Cc2Net87(args.fea_dim,model_name='Cc2Net87')
    elif 'DCRN_02' in net:
        model_g = DCRN_02(args.fea_dim,args.patches,model_name='DCRN_02')
    else:
        print('Please specify the backbone network')
