...
class GIDDM():
    ...
    def build_model_init(self):
        ...
        self.G=SS_FE( input_dim=102,output_dim=self.args.dim1,hidden_dim1=[self.args.dim2,self.args.dim3],hidden_dim2=[self.args.dim5,self.args.dim6],act= nn.Tanh(),args=self.args)
        ...
        while step < self.args.warmup_iter + 1:
            ...
            for batch_idx, (img_s, label_s) in enumerate(self.src_train_loader):
                ...
                if step%100==0:
                    ...
                    nni.report_intermediate_result(aa)
                    if aa > best:
                        best = aa
                    print("best:",best)
        ...
        for epoch in range(1, self.args.training_iter):
            ...
            for batch_idx, ((img_s, label_s), (img_t, label_t)) in enumerate(joint_loader):
                ...
                for _ in range(self.args.update_freq_G):
                    ...
                    W_distance=torch.sum(torch.abs(self.GCN(teacher_probs, edge_index.cuda()).detach()-self.GCN( student_probs, edge_index.cuda())))
            ...
            if (epoch%self.args.update_term==0):
                ...
                if C_acc_os * 100 > best and C_acc_unknown*100>20:
                    best = C_acc_os * 100
    ...
    def test(self, epoch, args=None, draw=False):
        ...
        m = utils.extended_confusion_matrix(total_label_t, total_pred_t, true_labels=list(range(max_target_label)),
        ...
        acc_unknown = sum([cm[i][known_num_class] for i in range(known_num_class, int(np.max(total_label_t) + 1))]) / (
        ...
        # if acc_os*100>84:
        #
        #     np.save(str('./tsne_data/PA/'+'feature_acc_os:'+str(round(acc_os*100,1))+'acc_uk:'+str(round(acc_unknown*100,1))+'.npy'),features.cpu().detach().numpy())
        #     np.save(
        #             str('./tsne_data/PA/' + 'label_acc_os:' + str(round(acc_os * 100, 1)) + 'acc_uk:' + str(
