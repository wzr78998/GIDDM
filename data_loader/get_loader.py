...
def get_dataset_information(dataset, s_d, t_d):
    if dataset == 'office':
        ...
        source_path = os.path.join(data_path, 'Houston13.mat')
        target_path = os.path.join(data_path, 'Houston18.mat')
        evaluation_data = os.path.join(data_path, 'Houston18.mat')
        class_list = ["grass healthy", "grass stressed", "trees", "water", "residential buildings",  "unk"]
        num_class = len(class_list)  # 7
    elif dataset == 'Pavia':
        s_d, t_d = 'train', 'validation'
        data_path = os.path.join('data', dataset)
        source_path = os.path.join(data_path, 'paviaC.mat')
        target_path = os.path.join(data_path, 'paviaU.mat')
        evaluation_data = os.path.join(data_path, 'paviaU.mat')
        class_list = ["trees", "Asphalt", "Brick", "Bitumen", "Shadow", "unk"]
        num_class = len(class_list)  # 7
    elif dataset == 'KSC':
        s_d, t_d = 'train', 'validation'
        data_path = os.path.join('data', dataset)
        source_path = os.path.join(data_path, 'ksc1_std.mat')
        target_path = os.path.join(data_path, 'ksc3_std.mat')
        evaluation_data = os.path.join(data_path, 'ksc3_std.mat')
        class_list = ["Water", "Scrub", "Slash pine", "Harwood swamp", "Willow swamp", "Graminoid marsh","Salt marsh","unk"]
        num_class = len(class_list)  # 7
