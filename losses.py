...
class stMMD_loss(nn.Module):
    def __init__(self, class_num=5, kernel_type='rbf', kernel_mul=2.0, kernel_num=5, fix_sigma=None,temperature=0.07,base_temperature=0.07):
    ...
    def push_away_UNK(self, source, target, s_label, t_label):
        UNK_loss = 0
        device = source.device  # 获取源特征所在的设备

        # 找到源域和目标域中所有唯一的类别
        unique_classes = torch.unique(torch.cat((s_label, t_label)).cpu().data).to(device)

        # 找到仅在目标域中存在的未知类别
        s_label_unique = torch.unique(s_label).to(device)
        unknown_classes = torch.isin(unique_classes, t_label) & ~torch.isin(unique_classes, s_label_unique)

        for class_idx in unknown_classes.nonzero(as_tuple=True)[0]:  # 使用nonzero获取索引
            # 找到未知类别在目标域中的索引
            ti_indices = (t_label == class_idx.item()).nonzero().view(-1)
            # 如果目标域中存在该未知类别
            if ti_indices.numel() > 0:
                # 提取目标域中未知类别的特征
                ti = target[ti_indices]
                # 计算目标域未知类别与源域所有类别之间的距离
                dist_source = torch.cdist(ti, source, p=2)
                # 计算目标域未知类别与目标域已知类别之间的距离
                mask_known = t_label != class_idx.item()
                dist_target_known = torch.cdist(ti, target[mask_known], p=2)

                # 将距离拉大
                UNK_loss -= torch.mean(dist_source)
                UNK_loss -= torch.mean(dist_target_known)
    ...
    def get_loss_cate(self, source, target, s_label, t_label):
        ...
        for index in indices:
            ...
            loss += torch.sum(TT)+torch.sum(SS) - torch.sum(ST) - torch.sum(TS)
            # loss += torch.sum(SS) + torch.sum(TT) - torch.sum(ST) - torch.sum(TS)
            # loss += (torch.sum(SS) + torch.sum(TT) - torch.sum(ST) - torch.sum(TS))/(si_batch_size+ti_batch_size)
